...
I will put them in a list "a", also write an anonymous function where data frame will be filtered by the brand name input, then I used the `ggplot` function, assigning "rating" as y and "usd_price" as x. Later I will add a `geom_point` layer to generate the corresponding scatter plot. Finally I will use a `lapply` function to apply the anonymous function to every element in the list "a". 
